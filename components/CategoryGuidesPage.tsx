// app/guides/components/CategoryGuidesPage.tsx
"use client";
// @ts-nocheck
...
import { guidesData } from "@/app/guides/data/guides";
...
const categories = [
  { id: "all", label: "All Guides", icon: "📖", href: "/guides" },
  { id: "phones", label: "Phones", icon: "📱", href: "/guides/phones" },
  { id: "laptops", label: "Laptops", icon: "💻", href: "/guides/laptops" },
  { id: "auto", label: "Auto", icon: "🚗", href: "/guides/auto" },
  { id: "technology", label: "Technology", icon: "💡", href: "/guides/technology" },
];

export function CategoryGuidesPage({ category, icon, gradient, title, description }: CategoryGuidesPageProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");

  // Filter guides by category and search
  const guides = useMemo(() => {
    let list = Object.values(guidesData)
      ...
      <main className="wrap py-6">
        ...
        <section className={`relative rounded-[20px] overflow-hidden mb-8 bg-gradient-to-br ${gradient} text-white`}>
          <div className="relative z-10 p-8 md:p-11">
            <div className="max-w-[700px]">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-[0.7rem] font-jetbrains-mono uppercase tracking-[0.15em] bg-white/15 px-3 py-1 rounded-full font-semibold">
                  {title} Guides
                </span>
              </div>
              <h1 className="font-fraunces font-medium text-[clamp(2.2rem,4.5vw,3.5rem)] tracking-[-0.03em] leading-[1.1]">
                {title} <em className="italic not-italic text-[#D4F26B]">Buying Guides</em>
              </h1>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-30" viewBox="0 0 800 400" preserveAspectRatio="none">
            ...
            <path d="M100 350 L250 150 L400 280 L550 100 L700 200" stroke="rgba(255,255,255,0.04)" strokeWidth="2" fill="none" />
          </svg>
        </section>
                ...
                >
                  <div className="relative w-full aspect-[16/9] overflow-hidden bg-[#eef1e9]">
                    ...
                    <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1.5 z-10">
                      ...
                      <span className={`text-[0.5rem] px-2 py-0.5 rounded-full font-bold uppercase tracking-[0.05em] font-jetbrains-mono border ${getDifficultyColor(guide.level)}`}>
                        {guide.level}
                      </span>
                    </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[0.6rem] font-semibold text-[var(--color-ink-soft)]">{guide.categoryIcon}</span>
                      <span className="text-[0.55rem] font-medium text-[var(--color-ink-soft)]">{guide.categoryLabel}</span>
                      <span className={`text-[0.5rem] px-2 py-0.25 rounded-full font-bold uppercase tracking-[0.05em] font-jetbrains-mono border ${getDifficultyColor(guide.level)}`}>
                        {guide.level}
                      </span>
                      <span className="text-[0.5rem] text-[var(--color-ink-soft)] font-jetbrains-mono">⏱️ {guide.readTime}</span>
                    </div>
