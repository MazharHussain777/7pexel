  ...
  return (
    ...
    >
      ...
      <div className="px-1 py-1.5 text-center border-t border-[rgba(127,1,31,0.04)]">
        ...
        <span className="text-[0.4rem] font-bold uppercase tracking-[0.06em] text-[#7F011F]/60 block mb-0.5">
          {phone.brand}
        </span>
        {/* Phone Name */}
        <span className="text-[0.6rem] font-medium text-[#2d1a1a] font-['Poppins',sans-serif] line-clamp-2 group-hover:text-[#7F011F] transition-colors leading-tight block">
          {phone.name}
        </span>
      </div>
    </Link>
...
export function RelatedPhonesGrid({ relatedPhones, currentSlug }: RelatedPhonesGridProps) {
  ...
  return (
    <section className="mt-6" aria-labelledby="related-phones-title">
      {/* Section Header */}
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          <div className="w-0.5 h-3.5 bg-gradient-to-b from-[#7F011F] to-[#a80a30] rounded-full" />
          <h2 className="text-[0.6rem] font-semibold uppercase tracking-[0.08em] text-[#2d1a1a]/70 font-['Poppins',sans-serif]">
            Related
          </h2>
          <span className="text-[0.45rem] font-medium text-[#6d4a4a]/50 bg-[#f5ebd0]/30 px-1.5 py-0.5 rounded-full">
            {displayPhones.length}
          </span>
        </div>
        <Link
          href="/phone-finder"
          className="text-[0.45rem] font-medium text-[#7F011F]/70 hover:text-[#7F011F] transition-colors flex items-center gap-1 group"
        >
          View All
          <i className="fas fa-arrow-right text-[0.35rem] group-hover:translate-x-0.5 transition-transform" />
        </Link>
      </div>
