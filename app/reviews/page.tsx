...
export default function ReviewsPage() {
  ...
  const [sortMode, setSortMode] = useState<"rating" | "newest" | "oldest">("newest");
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [categoriesRes, reviewsRes, tagsRes] = await Promise.all([
          fetch('/api/reviews/categories'),
          fetch('/api/reviews'),
          fetch('/api/reviews/tags')
        ]);
  ...
  const filteredReviews = useMemo(() => {
    ...
    if (activeCategory !== "all") {
      list = list.filter((r) => r.categorySlug === activeCategory);
    }
    ...
    return list;
  }, [reviews, activeCategory, searchQuery, sortMode, selectedTag]);
    ...
    return (
      <div className="min-h-screen bg-[#fbfdfb]">
        ...
        <main className="wrap py-20 text-center">
          ...
          <p className="text-[var(--color-ink-soft)]">Loading reviews...</p>
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#fbfdfb]">
      ...
      <main className="wrap py-6">
        {/* ─── BREADCRUMB ──────────────────────────────── */}
        <div className="flex items-center gap-2 text-[0.8rem] text-[var(--color-ink-soft)] mb-4 flex-wrap mt-5">
          <Link href="/" className="hover:text-[var(--color-green)] transition-colors">Home</Link>
          <span className="opacity-40">/</span>
          <span className="text-[var(--color-ink)] font-semibold">Reviews</span>
        </div>
        ...
        <section className="relative rounded-[24px] overflow-hidden mb-8 bg-gradient-to-br from-[#0A3F26] via-[#0F6B3E] to-[#1FA25A] text-white">
          <div className="relative z-10 p-10 md:p-14">
            <div className="max-w-[700px]">
              ...
              <h1 className="font-fraunces font-medium text-[clamp(2.5rem,5vw,4rem)] tracking-[-0.03em] leading-[1.08]">
                Honest <em className="italic not-italic text-[#D4F26B]">Reviews</em> for Real People
              </h1>
              <p className="mt-4 text-white/85 text-[1.05rem] leading-[1.7] max-w-[600px]">
                Read expert reviews of phones, laptops, cars, and tech products. Unbiased ratings, pros & cons, and buying advice to help you make the right decision.
              </p>
              ...
              <div className="mt-7 max-w-[500px] relative">
                ...
                <input
                  type="text"
                  placeholder="Search reviews by brand, model, or category..."
                  value={searchQuery}
              ...
              <div className="flex gap-8 flex-wrap mt-8">
                <div>
                  <div className="font-fraunces font-semibold text-2xl text-white">{reviews.length}</div>
                  <div className="text-[0.6rem] uppercase tracking-[0.08em] text-white/70">Total Reviews</div>
                </div>
                <div className="border-l border-white/20 pl-8">
                  <div className="font-fraunces font-semibold text-2xl text-white">{categories.length}</div>
                  <div className="text-[0.6rem] uppercase tracking-[0.08em] text-white/70">Categories</div>
                </div>
                <div className="border-l border-white/20 pl-8">
                  <div className="font-fraunces font-semibold text-2xl text-white">{featuredReviews.length}</div>
                  <div className="text-[0.6rem] uppercase tracking-[0.08em] text-white/70">Featured</div>
                </div>
                <div className="border-l border-white/20 pl-8">
                  <div className="font-fraunces font-semibold text-2xl text-white">{allTags.length}</div>
                  <div className="text-[0.6rem] uppercase tracking-[0.08em] text-white/70">Topics</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20" viewBox="0 0 800 400" preserveAspectRatio="none">
            ...
            <path d="M50 200 L200 100 L350 180 L500 80 L650 160 L750 100" stroke="rgba(255,255,255,0.03)" strokeWidth="2" fill="none" />
          </svg>
        </section>
        ...
        {/* ─── FEATURED REVIEWS ────────────────────────── */}
        {activeCategory === "all" && featuredReviews.length > 0 && (
                ...
                >
                  <div className="relative w-full aspect-[16/9] overflow-hidden bg-[#eef1e9]">
                    ...
                    <span className="absolute bottom-3 left-3 text-[0.5rem] px-2 py-0.5 rounded-full bg-[var(--color-green)] text-white font-bold uppercase tracking-[0.05em]">
                      {review.categoryIcon} {review.categoryLabel}
                    </span>
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-[0.75rem] text-[var(--color-ink-soft)] mt-1.5 line-clamp-2">{review.excerpt}</p>
                    <div className="flex items-center gap-2 mt-3 pt-2.5 border-t border-dashed border-[var(--color-line)]">
                      ...
                      <span className="text-[0.6rem] font-medium">{review.author}</span>
                      <span className="text-[0.55rem] text-[var(--color-ink-soft)]">· {review.readTime}</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <section>
          <div className="flex justify-between items-baseline mb-5 flex-wrap gap-2.5">
            <h2 className="font-fraunces font-medium text-[1.5rem] tracking-[-0.01em]">
              ...
              <em className="italic not-italic text-[var(--color-green)] ml-1">Reviews</em>
            </h2>
            <span className="text-[0.7rem] text-[var(--color-ink-soft)]">{filteredReviews.length} reviews</span>
          </div>
          ...
          <div className="border-[1.5px] border-[var(--color-line)] rounded-[14px] p-4.5 mb-6 bg-[var(--color-paper)]">
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <select
                ...
                className="px-3.5 py-1.5 rounded-full border-[1.5px] border-[var(--color-line)] bg-white text-[0.75rem] font-semibold text-[var(--color-ink)]"
              >
          ...
          {/* ─── GRID VIEW ────────────────────────────── */}
          {viewMode === "grid" && filteredReviews.length > 0 && (
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[0.6rem] font-semibold text-[var(--color-ink-soft)]">{review.brand}</span>
                      <span className="text-[0.5rem] text-[var(--color-ink-soft)]">· {review.model}</span>
                    </div>
                    <h4 className="font-fraunces font-medium text-[1rem] leading-[1.3] group-hover:text-[var(--color-green)] transition-colors line-clamp-2">
                      {review.title}
                    </h4>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[0.75rem]">{getStars(review.rating)}</span>
                      <span className="text-[0.65rem] font-bold text-[var(--color-green)]">{review.rating}/5</span>
                    </div>
                    <p className="text-[0.75rem] text-[var(--color-ink-soft)] mt-1.5 line-clamp-2">{review.excerpt}</p>
                    <div className="flex items-center gap-2 mt-3 pt-2.5 border-t border-dashed border-[var(--color-line)]">
                      <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[var(--color-green-deep)] to-[var(--color-green-bright)] flex items-center justify-center text-white font-semibold text-[0.5rem] flex-shrink-0">
          ...
          {/* ─── LIST VIEW ────────────────────────────── */}
          {viewMode === "list" && filteredReviews.length > 0 && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className="text-[0.5rem] text-[var(--color-ink-soft)]">{review.brand}</span>
                      <span className="text-[0.5rem] text-[var(--color-ink-soft)]">· {review.model}</span>
                      {review.isTrending && (
                    ...
                    <p className="text-[0.8rem] text-[var(--color-ink-soft)] mt-1 line-clamp-2">{review.excerpt}</p>
          ...
          {filteredReviews.length === 0 && (
            <div className="flex flex-col items-center text-center gap-3.5 py-16 px-5 text-[var(--color-ink-soft)]">
              ...
              <h3 className="text-xl font-medium text-[var(--color-ink)]">No reviews found</h3>
